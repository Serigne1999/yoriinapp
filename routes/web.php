...
//demo yoriin application


Route::get('/demo-login', function () {
    $user = User::where('email', 'admin@yoriinapp.com')->first();

    if (!$user) {
        abort(404, 'Compte démo introuvable.');
    }

    Auth::login($user);

    return redirect('/home'); // ou '/dashboard' selon ta config
});
...
Route::get('/generate-sitemap', function () {
    ...
    // Enregistrer le fichier sitemap.xml dans le dossier public
    File::put(public_path('sitemap.xml'), $xml);

    return response('Sitemap généré avec succès !')->header('Content-Type', 'text/plain');
});
...
Route::middleware(['setData', 'auth', 'SetSessionData', 'language', 'timezone', 'AdminSidebarMenu', 'CheckUserLogin'])->group(function () {
    ...
    // Route::resource('payment-account', 'PaymentAccountController'); // DÉSACTIVÉ - Contrôleur manquant
