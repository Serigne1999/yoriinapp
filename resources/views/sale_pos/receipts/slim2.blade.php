            ...
            <table style="padding-top: 5px !important" class="border-bottom width-100 table-f-12 mb-10">
                <tbody>
                    @forelse($receipt_details->lines as $line)
                        ...
                        <tr>
                            <td colspan="3" class="border-bottom-dotted"></td>
                        </tr>
                        
                        <!-- Numéro + Nom du produit complet sur une ligne -->
                        <tr class="description">
                            <td colspan="3" class="description">
                                <p class="product-name m-0">{{$loop->iteration}}. {{$line['name']}}</p>
                            </td>
                        </tr>
                        
                        <!-- Quantité x Prix et Total sur la même ligne -->
                        <tr>
                            <td colspan="3">
                                @if(empty($receipt_details->hide_price))
                                <div style="display: flex; justify-content: space-between; width: 100%;">
                                    <p class="product-details m-0">{{$line['quantity']}} x {{$line['unit_price_before_discount']}}</p>
                                    <p class="product-details m-0" style="text-align: right;">{{$line['line_total']}}</p>
                                </div>
                                @else
                                <p class="product-details m-0">{{$line['quantity']}}</p>
                                @endif
                            </td>
                        </tr>
                        
                        <!-- Modificateurs si présents -->
                        @if(!empty($line['modifiers']))
                            @foreach($line['modifiers'] as $modifier)
                                <tr>
                                    <td colspan="3">
                                        <p class="modifier-name m-0">+ {{$modifier['name']}}</p>
                                    </td>
                                </tr>
                                <tr>
                                    <td colspan="3">
                                        @if(empty($receipt_details->hide_price))
                                        <div style="display: flex; justify-content: space-between; width: 100%;">
                                            <p class="product-details m-0">{{$modifier['quantity']}} x {{$modifier['unit_price_inc_tax']}}</p>
                                            <p class="product-details m-0" style="text-align: right;">{{$modifier['line_total']}}</p>
                                        </div>
                                        @endif
                                    </td>
                                </tr>
...
/* Styles pour le nom du produit (description) */
.product-name {
	font-size: 14px;
	font-weight: 700;
...
/* Styles pour la ligne PU | Qte | Tt */
.product-details {
	font-size: 13px;
	font-weight: 700;
...
/* Total Quantité et Total Items */
.total-quantity {
	font-size: 18px;
	font-weight: 700;
}

/* Total général (Subtotal, Total, Total payé) */
.total-amount {
	font-size: 18px;
	font-weight: 700;
}
